...
class ReviewsController extends Controller
{
    public function index()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            } elseif ($serviceId > 0 && !empty($comment)) {
                if (Review::add($ownerId, $serviceId, $rating, $comment)) {
                    $_SESSION['review_submitted'] = true;
                } else {
                    $_SESSION['review_error'] = 'Failed to submit review.';
                }
            } else {
                $_SESSION['review_error'] = 'Please select a service and write a comment.';
            }
        ...
        $this->view('reviews/index', compact('reviews', 'services', 'pageTitle'));
    }
}  